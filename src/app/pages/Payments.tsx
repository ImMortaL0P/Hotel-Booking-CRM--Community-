...
export function Payments() {
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      ...
      case 'Refunded': return 'text-foreground/80 bg-muted';
    ...
    <div className="space-y-6 h-full flex flex-col relative print:m-0 print:p-0 print:block">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden">
        <div>
          <div className="flex flex-col">
            ...
            <p className="text-sm text-muted-foreground">Track collections, outstanding balances, and generate receipts.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 shrink-0 print:hidden">
        ...
        <div className="bg-card p-5 rounded-lg border border-border border-l-4 border-l-muted-foreground flex flex-col justify-between">
          ...
          <div className="text-3xl font-bold text-foreground">{formatCurrency(0)}</div>
        </div>
      </div>
            ...
            {filteredPayments.map(p => {
              ...
              return (
                <tr key={p.id} className="hover:bg-muted/50">
                  ...
                  <td className="px-4 py-3 font-semibold text-primary hover:underline cursor-pointer">{p.bookingId}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <button 
                      ...
                      className="p-1.5 text-primary hover:bg-red-50 rounded" title="View Full Receipt"
                    >
        ...
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 print:block print:relative print:bg-transparent print:p-0 print:inset-auto">
            <div className="bg-card rounded-xl shadow-2xl max-w-2xl w-full flex flex-col max-h-full overflow-hidden animate-in zoom-in-95 duration-200 print:shadow-none print:max-w-none print:border-none print:overflow-visible">
              ...
              {/* Printable Area */}
              <div className="p-8 overflow-y-auto bg-card" id="printable-receipt">
                <div className="text-center mb-6">
                  <h1 className="text-3xl font-bold text-primary uppercase tracking-wider font-serif">Sharda Palace</h1>
                  <p className="text-sm text-foreground/80 mt-1">Main Shivganga Road, Bam Bam Baba Path, near Matri Mandir<br/>Deoghar, Jharkhand 814112</p>
                ...
                <div className="flex justify-between items-end border-b-2 border-primary pb-4 mb-6">
                  <div>
                    ...
                    <p className="text-muted-foreground font-medium text-sm">Date: {formatDate(selectedReceipt.date)} {new Date(selectedReceipt.date).toLocaleTimeString()}</p>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-8 mb-8">
                  <div>
                    <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Billed To</h4>
                    <p className="font-bold text-foreground text-lg">{_guest?.name || 'Walk-in Guest'}</p>
                    {_guest && (
                  ...
                  <div className="text-right flex flex-col justify-end">
                    <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Booking Reference</h4>
                    <p className="font-bold text-primary text-lg">{selectedReceipt.bookingId}</p>
                    <p className="text-foreground/80 text-sm mt-1 border inline-block ml-auto px-2 py-0.5 rounded bg-muted/50 mt-2 font-medium">
                ...
                <table className="w-full mb-8">
                  <thead className="bg-secondary border-y border-border">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-center text-sm text-foreground">Payment Mode</th>
                      <th className="px-4 py-2 text-right text-sm text-foreground">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td className="px-4 py-4 text-sm font-medium border-b border-border/50">
                        Accommodation / Services Advance
                      </td>
                      <td className="px-4 py-4 text-sm text-center border-b border-border/50">
                         <span className="font-mono text-xs font-bold">{selectedReceipt.mode}</span>
                      </td>
                      <td className="px-4 py-4 text-sm text-right font-bold text-foreground border-b border-border/50">
                  ...
                  <tfoot>
                    <tr>
                      <td colSpan={2} className="px-4 py-3 text-right font-bold text-foreground">Total Paid</td>
                      <td className="px-4 py-3 text-right font-bold text-xl text-primary">{formatCurrency(selectedReceipt.amount)}</td>
                    </tr>
                  </tfoot>
                </table>
